...
Moved to [UPGRADING.md](UPGRADING.md) — everything an operator has to do or
check before deploying this round, including the one change that can take a
fleet down.
...
Theme: server-to-server endpoints must not look like authentications.
Operators running the CrowdSec agent should upgrade.

### pam-access

- **Fix — a bastion could be made to ban itself from its own SSO, and more
  generally trigger any alerting plugin.**
...
- **Feature — Cross-App Access (XAA)**, both ends of
  ...
  as two separately autoloaded modules. Issuing side: a confidential client
  exchanges one of its tokens against a short-lived `oauth-id-jag+jwt`
  assertion targeting another authorization server. Consuming side: a client
  redeems such an assertion with the RFC 7523 JWT Bearer grant and gets a
  local access token, after signature, audience, freshness, client and
  single-use `jti` checks.
- RFC 9396 `authorization_details` travel with the assertion, narrowed by the
  target relying party's type allowlist. An ID token used as `subject_token`
  no longer requires the client to be allowed refresh tokens.
- Discovery advertises the token exchange and jwt-bearer grants and
  `identity_chaining_requested_token_types_supported`; the new
  `oidcGenerateIdJag` hook lets other plugins enrich the assertion.
